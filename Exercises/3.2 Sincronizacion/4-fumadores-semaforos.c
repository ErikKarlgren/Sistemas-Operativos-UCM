...
void fumadorConPapel(void)
{
    while (1 == 1)
    {
        // bloqueamos el acceso a 'ingredientes'
        sem_wait(&ingrs_sem);

        // espero mientras no haya cerillas y tabaco
        while (!ingredientes[TABACO] || !ingredientes[CERILLAS])
        {
            // desbloqueamos el semáforo
            sem_post(&ingrs_sem);
            // y esperamos a que 'agente' nos despierte
            sem_wait(&smokers_sem);
            // cuando despertemos, volvemos a bloquear o a esperar
            sem_wait(&ingrs_sem);
        }
        // cojo el tabaco
        ...
        // desbloqueamos el acceso a 'ingredientes'
        sem_post(&ingrs_sem);
...
void agente(void)
{
    ...
    while (1 == 1)
    {
        ...
        // vamos a leer 'ingredientes' así que bloqueamos
        sem_wait(&ingrs_sem);
        ...
        sem_getvalue(&smokers_sem, &waiting_smokers);
