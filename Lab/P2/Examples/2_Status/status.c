...
typedef enum { STAT_MODE, LSTAT_MODE } status_mode;

char permissions[] = {'x', 'w', 'r'};

int status(char *, status_mode);

int main(int argc, char *argv[]) {
  int i;
  status_mode mode = STAT_MODE;
  ...
  for (i = mode == LSTAT_MODE ? 1 : 2; i < argc; i++)
    status(argv[i], mode);

  exit(0);
}

int status(char *filename, status_mode mode) {
  struct stat buf;
  struct passwd *pw;
  struct group *gr;
  int i;
  ...
  if (mode == STAT_MODE && stat(filename, &buf) == -1)
    perror(filename), exit(-1);
  else if (mode == LSTAT_MODE && lstat(filename, &buf) == -1)
    perror(filename), exit(-1);
  printf("File: %s\n", filename);

  // The st_dev field describes the device on which this file resides.
  // The following technique is useful to decompose the device ID (pair
  // major-minor) in this field.
  printf("Resides in the device: %d, %d\n", (int)(buf.st_dev & 0xff00) >> 8,
         (int)buf.st_dev & 0x00ff);
