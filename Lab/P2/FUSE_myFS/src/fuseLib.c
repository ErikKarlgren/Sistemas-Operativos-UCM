...
static int my_getattr(const char *path, struct stat *stbuf)
{
    ...
    {
        stbuf->st_mode = S_IFREG | 0444; // can only be read
        stbuf->st_size = BLOCK_SIZE_BYTES;
        stbuf->st_uid = getuid();
        stbuf->st_gid = getgid();
        stbuf->st_mtime = stbuf->st_ctime = time(0);
        return 0;
    }*/
...
/**** Operations added by the student ****/

/**
 * @brief Read a file
 *
 * @param path file path
 * @param buf buffer where contents from the file are copied to
 * @param size number of bytes to be copied at most
 * @param offset offset from the beginning of the file where this function starts reading
 * @param f file info
 * 
 * @return 0 on success and < 0 on error
 **/
...
{
    char buffer[BLOCK_SIZE_BYTES];
    int bytes2Read, totalRead = 0, bytesAvailable2BeRead;
    ...
    {
        fprintf(stderr, "Offset higher than file size in my_write\n");
        return -EIO;
    }

    bytesAvailable2BeRead = node->fileSize - offset;
    bytes2Read = size < bytesAvailable2BeRead ? size : bytesAvailable2BeRead;
    ...
    while (totalRead < bytes2Read)
    {
        ...
        // buffer = block indicated by currentBlock
        if (readBlock(&myFileSystem, currentBlock, &buffer) == -1)
        {
        ...
        // we copy to buf
        for (i = offset; (i < BLOCK_SIZE_BYTES) && (totalRead < bytes2Read); i++)
        {
...
/**
 * @brief Unlinks a file and deletes it
 *
 * @param path file path
 * 
 * @return 0 on success and < 0 on error
...
{
    int idxNode, dirFileIndex = 0;
    NodeStruct *inode;

    fprintf(stderr, "--->>>my_unlink: path %s\n", path);

    // for findFileByName() we need to ignore the 1st character (a '/')
    path++;

    // We search the file
    dirFileIndex = findFileByName(&myFileSystem, path);
    fprintf(stderr, "   Final file index %d\n", dirFileIndex);

    // we fix the pointer since we modified it earlier, although it might be unnecessary
    path--;

    if (dirFileIndex == -1) // file doesn't exist or there are no files in the directory
    {
        fprintf(stderr, "Error: file not found in my_unlink\n");
        return -EIO;
    }

    idxNode = myFileSystem.directory.files[dirFileIndex].nodeIdx;
    inode = myFileSystem.nodes[idxNode];
    fprintf(stderr, "   i-node %d\n", idxNode);

    // truncate file
    if (resizeNode(idxNode, 0) < 0)
    {
        fprintf(stderr, "Error while truncating file in my_unlink\n");
        return -EIO;
    }

    // mark as free file and decrease number of files in directory
    myFileSystem.directory.files[dirFileIndex].freeFile = true;
    myFileSystem.directory.numFiles--;
    fprintf(stderr, "   Decreased number of files to %d\n", myFileSystem.directory.numFiles);

    // mark i-node as free and increase number of free nodes
    inode->freeNode = true;
    myFileSystem.numFreeNodes++;
    fprintf(stderr, "   Increased number of free nodes to %d\n", myFileSystem.numFreeNodes);

    // update directory
    if (updateDirectory(&myFileSystem))
    {
        fprintf(stderr, "Error while updating directory in my_unlink\n");
        return -EIO;
    }
    fprintf(stderr, "   Updated directory\n");

    // update i-nodes
    if (updateNode(&myFileSystem, idxNode, inode))
    {
        fprintf(stderr, "Error while updating node in my_unlink\n");
        return -EIO;
    }
    fprintf(stderr, "   Updated i-nodes and freed file's i-node\n");
